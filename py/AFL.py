...
def draw():
    ...
    # 绘图
    plt.plot(time_list5, cov5, color='skyblue', label='afl_strip-new')
    plt.plot(time_list4, cov4, color='black', label='afl_size')
    plt.plot(time_list3, cov3, color='blue', label='afl_readelf')
    plt.plot(time_list2, cov2, color='red', label='afl_objdump')
    plt.plot(time_list1, cov1, color='green', label='afl_nm-new')  # 传入x，y绘制出折线图
    # 显示图例
    plt.legend()
    # 横纵轴标签
    ...
    # 保存图片
    plt.savefig("./afl_cov.png")  # 保存图片


if __name__ == '__main__':
    image = read('afl_out_nm-new/plot_data')
    time_list1 = []
    cov1 = []
    ...
    image = read('afl_out_objdump/plot_data')
    time_list2 = []
    cov2 = []
    start_time = datetime.strptime(trans_time(int(image[0][0])), "%Y-%m-%d %H:%M:%S")
    cnt = 0
    ...
    image = read('afl_out_readelf/plot_data')
    time_list3 = []
    cov3 = []
    start_time = datetime.strptime(trans_time(int(image[0][0])), "%Y-%m-%d %H:%M:%S")
    cnt = 0
    ...
    image = read('afl_out_size/plot_data')
    time_list4 = []
    cov4 = []
    start_time = datetime.strptime(trans_time(int(image[0][0])), "%Y-%m-%d %H:%M:%S")
    cnt = 0
    ...
    image = read('afl_out_strip-new/plot_data')
    time_list5 = []
    cov5 = []
    start_time = datetime.strptime(trans_time(int(image[0][0])), "%Y-%m-%d %H:%M:%S")
    cnt = 0
